...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit'])) {
    ...
    // Insert into staff (no photo field)
    $stmt = $mysqli->prepare("INSERT INTO staff (id, name, phone, email, sex, dob, hiredate, address, salary, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, NOW(), NOW())");
    $stmt->bind_param("ssssssssd", $Id, $Name, $Phone, $Email, $gender, $DOB, $HireDate, $Address, $Salary);
    $ok1 = $stmt->execute();
    $stmt->close();
    // Insert into users
    $stmt2 = $mysqli->prepare("INSERT INTO users (userid, password, usertype) VALUES (?, ?, 'staff')");
    $stmt2->bind_param("ss", $Id, $Password);
    $ok2 = $stmt2->execute();
    $stmt2->close();
    if ($ok1 && $ok2) {
        $successMsg = "Staff registered successfully.";
    } else {
        $errorMsg = "Error registering staff. Please check the details.";
    }
}
?>
<!DOCTYPE html>
...
<head>
    ...
    <title>Add Staff - Admin Panel | School Management System</title>
        ...
        <div class="form-container">
            ...
            <form action="" method="post">
                ...
                <div class="form-group">
                    ...
                    <input type="date" name="HireDate" value="<?= date('Y-m-d') ?>" required readonly>
                </div>
                <div class="form-group">
                    <label>Address</label>
